...
public class AgentAppService {
    ...
    public AgentAppService(AgentDomainService agentDomainService,AgentWorkspaceDomainService agentWorkspaceDomainService) {
    ...
    public AgentDTO createAgent(CreateAgentRequest request, String userId) {
        ...
        AgentEntity agent = agentDomainService.createAgent(entity);
    ...
    public List<AgentDTO> getUserAgents(String userId, SearchAgentsRequest searchAgentsRequest) {
        ...
        List<AgentEntity> agents = agentDomainService.getUserAgents(userId, entity);
    ...
    public List<AgentVersionDTO> getPublishedAgentsByName(SearchAgentsRequest searchAgentsRequest) {
        AgentEntity entity = AgentAssembler.toEntity(searchAgentsRequest);
        List<AgentVersionEntity> agentVersionEntities = agentDomainService.getPublishedAgentsByName(entity);
    ...
    public AgentDTO updateAgent(UpdateAgentRequest request, String userId) {
        ...
        // 调用领域服务更新Agent
        AgentEntity agentEntity = agentDomainService.updateAgent(updateEntity);
    ...
    public AgentDTO toggleAgentStatus(String agentId) {
        AgentEntity agentEntity = agentDomainService.toggleAgentStatus(agentId);
        return AgentAssembler.toDTO(agentEntity);
    }
    ...
    public AgentVersionDTO publishAgentVersion(String agentId, PublishAgentVersionRequest request, String userId) {
        ...
        // 获取最新版本，检查版本号大小
        AgentVersionEntity agentVersionEntity = agentDomainService.getLatestAgentVersion(agentId);
        ...
        // 调用领域服务发布版本
        agentVersionEntity = agentDomainService.publishAgentVersion(agentId, versionEntity);
    ...
    public List<AgentVersionDTO> getAgentVersions(String agentId, String userId) {
        List<AgentVersionEntity> agentVersions = agentDomainService.getAgentVersions(agentId, userId);
    ...
    public AgentVersionDTO getAgentVersion(String agentId, String versionNumber) {
        AgentVersionEntity agentVersion = agentDomainService.getAgentVersion(agentId, versionNumber);
    ...
    public AgentVersionDTO reviewAgentVersion(String versionId, ReviewAgentVersionRequest request) {
        ...
        if (PublishStatus.REJECTED.equals(request.getStatus())) {
            // 拒绝发布，需使用拒绝原因
            agentVersionEntity = agentDomainService.rejectVersion(versionId, request.getRejectReason());
        } else {
            // 其他状态变更，直接更新状态
            agentVersionEntity = agentDomainService.updateVersionPublishStatus(versionId, request.getStatus());
        }
        return AgentVersionAssembler.toDTO(agentVersionEntity);
    }
    ...
    public List<AgentVersionDTO> getVersionsByStatus(PublishStatus status) {
        List<AgentVersionEntity> versionsByStatus = agentDomainService.getVersionsByStatus(status);
